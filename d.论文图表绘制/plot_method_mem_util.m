function fig = plot_method_mem_util(data, savePath, smoothWin)
% PLOT_METHOD_MEM_UTIL  方法对比实验内存利用率折线图
%
%   fig = plot_method_mem_util(data)
%   fig = plot_method_mem_util(data, savePath)
%   fig = plot_method_mem_util(data, savePath, smoothWin)
%
...
if isempty(arrivalRate)
    warning('plot_method_mem_util: Number of Admitted SFC Requests >= 40 的数据为空，跳过绘图。');
...
fig = figure('Name','Method Comparison - Memory Utilization', 'Color','w', ...
...
ylabel('Memory Utilization',  'FontSize', fontSize, 'FontName', fontName);
...
if ~isempty(savePath)
    ...
    baseName = fullfile(savePath, 'Fig_MethodMemUtil');
    print(fig, baseName, '-dsvg', '-r0');
    print(fig, baseName, '-dpng', '-r600');
    fprintf('已保存：Fig_MethodMemUtil (.svg / .png) → %s\n', savePath);
end
end
