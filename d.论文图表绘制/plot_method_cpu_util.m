function fig = plot_method_cpu_util(data, savePath, smoothWin)
% PLOT_METHOD_CPU_UTIL  方法对比实验 CPU 利用率折线图
%
%   fig = plot_method_cpu_util(data)
%   fig = plot_method_cpu_util(data, savePath)
%   fig = plot_method_cpu_util(data, savePath, smoothWin)
%
...
if isempty(arrivalRate)
    warning('plot_method_cpu_util: Number of Admitted SFC Requests >= 40 的数据为空，跳过绘图。');
...
fig = figure('Name','Method Comparison - CPU Utilization', 'Color','w', ...
...
ylabel('CPU Utilization',  'FontSize', fontSize, 'FontName', fontName);
...
if ~isempty(savePath)
    ...
    baseName = fullfile(savePath, 'Fig_MethodCpuUtil');
    print(fig, baseName, '-dsvg', '-r0');
    print(fig, baseName, '-dpng', '-r600');
    fprintf('已保存：Fig_MethodCpuUtil (.svg / .png) → %s\n', savePath);
end
end
