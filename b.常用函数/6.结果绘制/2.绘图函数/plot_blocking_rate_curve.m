function plot_blocking_rate_curve(curve_tbl, outDir, cfg)
%PLOT_BLOCKING_RATE_CURVE  请求阻塞率变化曲线（双模式）
%
%   plot_blocking_rate_curve()                           -- 独立运行
%   plot_blocking_rate_curve(curve_tbl, outDir, cfg)     -- 批量调用
%
% 从"逐请求曲线"数据中读取 block_rate_mean 列绘制。
    ...
    if ~ismember('block_rate_mean', curve_tbl.Properties.VariableNames)
        warning('逐请求曲线中未找到 block_rate_mean 列'); return;
    end
    ...
    for m = 1:M
        ...
        grp  = curve_tbl(mask, :);
        plot(grp.success_index, grp.block_rate_mean, 'LineWidth', cfg.lineWidth);
    end
